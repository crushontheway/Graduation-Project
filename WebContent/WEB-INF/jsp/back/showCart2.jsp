...
<style>
	...
	function updateCart(input,id,oldvalue){
		...
		if(b){
			window.location.href="UpdateCart2?id="+id + "&itemnumber=" + itemnumber;
		}else{
			...
			<c:forEach items="${s_id_shop}" var="sitem" >
				...
				<c:forEach items="${shop_items[sitem.key]}" var="item">
					<tr>
						...
						<td>
							<input type="text" name="itemnumber" value="${item.itemnumber}" style="width: 30px;" onchange="updateCart(this,${item.id},${item.itemnumber})"/>
						</td>
						<td><a href="deleteItem2/${item.id}" style="text-decoration:none;">删除</a> </td>
					</tr>
				</c:forEach>
				<tr>
					<td colspan="2"><a href="addOrderPage2?s_id=${sitem.key}" style="text-decoration: none;">提交</a></td>
